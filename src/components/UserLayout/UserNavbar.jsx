import React from 'react'
import '../../styles/UserStylesheets/UserNavbar.css';

const UserNavbar = ({ collapsed, toggleSidebar }) => {
  const sidebarWidth = collapsed ? 70 : 250;

  return (
    <nav className="user-navbar" style={{ left: `${sidebarWidth}px`, width: `calc(100% - ${sidebarWidth}px)` }} >
      <div className="user-navbar-left">
        <div>
          <h4 className="mb-0 fw-bold">User Dashboard</h4>
          <small className="text-muted">Welcome back, User</small>
        </div>
      </div>

      <div className="user-profile">
        <div className="notification-icon">
          <span className="notification-badge">2</span>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-bell-fill" viewBox="0 0 16 16">
            <path d="M8 16a2 2 0 0 0 2-2H6a2 2 0 0 0 2 2m.995-14.901a1 1 0 1 0-1.99 0A5 5 0 0 0 3 6c0 1.098-.5 6-2 7h14c-1.5-1-2-5.902-2-7 0-2.42-1.72-4.44-4.005-4.901" />
          </svg>
        </div>

        <div className="profile-section d-flex align-items-center gap-3">

          <div className="dropdown">
            <button
              className="btn btn-outline-info dropdown-toggle thumbnail-btn"
              type="button"
              data-bs-toggle="dropdown"
              aria-expanded="false"
            >
              {/* Use a valid image link so it doesn't break into text */}
              <img
                src="https://randomuser.me/api/portraits"
                alt="User Profile"
                className="btn-thumbnail-img"
              />
            </button>
            <ul className="dropdown-menu">
              <li><a className="dropdown-item" href="#">My account</a></li>
              <li><a className="dropdown-item" href="#">Edit profile</a></li>
              <li><a className="dropdown-item" href="#">Settings</a></li>
              <li><hr className="dropdown-divider" /></li>
              <li><a className="dropdown-item text-danger" href="#">Sign out</a></li>
            </ul>
          </div>

          <div>
            <div className="fw-semibold">Pavan Kumar</div>
            <small className="text-muted">Software Developer</small>
          </div>

        </div>
      </div>


    </nav >
  )
}

export default UserNavbar
